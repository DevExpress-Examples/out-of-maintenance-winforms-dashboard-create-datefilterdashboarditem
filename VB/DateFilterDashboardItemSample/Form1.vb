...
Namespace DateFilterDashboardItemSample
	Partial Public Class Form1
		...
		Private Function CreateDateFilterItem(ByVal excelDataSource As DashboardExcelDataSource) As DateFilterDashboardItem
			...
			dateFilter.DateTimePeriods.AddRange(New DateTimePeriod With {
				.Name = DashboardWinLocalizer.GetString(DashboardWinStringId.PeriodLastYear), .Start = New FlowDateTimePeriodLimit With {.Interval = DateTimeInterval.Year, .Offset = -1},
				.End = New FlowDateTimePeriodLimit With {.Interval = DateTimeInterval.Year, .Offset = 0}
			},
			New DateTimePeriod With {
				.Name = DashboardWinLocalizer.GetString(DashboardWinStringId.PeriodNextSevenDays), .Start = New FlowDateTimePeriodLimit With {.Interval = DateTimeInterval.Day, .Offset = 1},
				.End = New FlowDateTimePeriodLimit With {.Interval = DateTimeInterval.Day, .Offset = 8}
			},
			New DateTimePeriod With {
				.Name = DashboardWinLocalizer.GetString(DashboardWinStringId.PeriodMonthToDate), .Start = New FlowDateTimePeriodLimit With {.Interval = DateTimeInterval.Month, .Offset = 0},
